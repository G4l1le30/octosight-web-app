...
class DashboardRepository:
    ...
    @staticmethod
    def team_summary(db: Session) -> list[dict[str, Any]]:
        """Return per-assignee ticket stats. Uses Python aggregation to avoid SQLAlchemy CASE issues."""
        from sqlalchemy import func as sa_func

        open_statuses = {"Submitted", "In Review"}
        ...
        for t in assigned_tickets:
            ...
            if uid not in by_user:
                by_user[uid] = {"total": 0, "open": 0, "high": 0, "response_seconds": []}
            by_user[uid]["total"] += 1
            ...
            if t.assigned_at and t.created_at:
                diff = (t.assigned_at - t.created_at).total_seconds()
                if diff >= 0:
                    by_user[uid]["response_seconds"].append(diff)

        result = []
        for uid, stats in by_user.items():
            user = db.query(User).filter(User.id == uid).first()
            avg_seconds = sum(stats["response_seconds"]) / len(stats["response_seconds"]) if stats["response_seconds"] else None
            result.append({
                "assignee_id": uid,
                "assignee_name": user.full_name if user else "Unknown",
                "assignee_role": user.role if user else None,
                "total_tickets": stats["total"],
                "open_tickets": stats["open"],
                "high_priority_tickets": stats["high"],
