    ...
    <div className="container mx-auto px-4 py-8 md:py-12 max-w-6xl">
      ...
      <div className="max-w-6xl mx-auto mt-16">
        <div className="relative">
          {/* Vertical Line */}
          {!loading && (
            <div className="absolute left-[24px] md:left-1/2 top-4 bottom-4 w-0.5 bg-neutral-border/80 md:-translate-x-1/2 z-0 hidden md:block"></div>
          )}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-full md:w-1/2 md:pr-12 z-10">
                      <div
                        ...
                        className={`card bg-white p-4 md:p-6 cursor-pointer border hover:border-primary hover:shadow-lg transition-all overflow-hidden relative ${
                          isLocked
                            ? "border-neutral-border bg-neutral-page/50"
                            : "border-neutral-border shadow-sm"
                        }`}
                      >
                        ...
                        <h3
                          className={`text-lg md:text-xl font-bold mb-4 leading-tight ${isLocked ? "text-secondary" : "text-secondary"}`}
                        >
                          {mod.title}
                        </h3>
                        ...
                        <div className="flex items-center justify-between mt-auto pt-4 border-t border-neutral-border/50">
                          ...
                          <span
                            className={`text-xs font-bold flex items-center gap-1 px-2 py-1 rounded-lg transition-all ${
                              isLocked
                                ? "text-secondary/60"
                                : "text-primary group-hover:bg-risk-high/10"
                            }`}
                          >
                            {isLocked ? (
                              <>
                        ...
                        <div className="absolute bottom-0 left-0 right-0 h-1.5 bg-neutral-border/50">
                          {(() => {
                            ...
                            const completedItems =
                              (mod.articles?.filter((a) => a.is_read).length ||
                                0) + (isCompleted ? 1 : 0);
                    ...
                    <div className="w-full md:w-1/2 md:pl-12 hidden md:flex flex-col justify-center py-4">
                      <h3
                        className={`text-lg md:text-xl font-bold mb-3 ${isLocked ? "text-secondary" : "text-secondary"}`}
                      >
                        {subTitle}
                      </h3>
                      <p
                        className={`font-medium leading-relaxed ${isLocked ? "text-secondary/60" : "text-secondary-light"}`}
                      >
                        {mod.description}
                      </p>
                    </div>
