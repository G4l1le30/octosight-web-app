    ...
    switch (formData.type) {
      case "SMS":
        return {
          ...
          summaryPlaceholder: "I received an SMS from this number saying my bank account was blocked...",
      ...
      case "WhatsApp":
        return {
          ...
          summaryPlaceholder: "Someone sent a WhatsApp message with a link to a fake login page...",
      ...
      case "Email":
        return {
          ...
          summaryPlaceholder: "I received a suspicious email asking me to verify my identity...",
    ...
    <div className="container mx-auto px-4 py-12 max-w-3xl">
      ...
      <div className="card p-8 shadow-md">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label htmlFor="url" className="text-sm font-black uppercase tracking-wider text-secondary/60">{dynamic.label}</label>
            <input 
              type="text" 
              id="url" 
          ...
          {(formData.type === 'SMS' || formData.type === 'WhatsApp' || formData.type === 'Email') && (
            <div className="space-y-2">
              <label htmlFor="senderNumbers" className="text-sm font-black uppercase tracking-wider text-secondary/60">{dynamic.senderLabel}</label>
              <input 
                type="text" 
                id="senderNumbers" 
                ...
                onChange={(e) => setFormData({...formData, senderNumbers: e.target.value})}
              />
              <p className="text-xs text-secondary-light">Separate multiple with commas</p>
            </div>
          )}

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3">
              <label className="text-sm font-black uppercase tracking-wider text-secondary/60">{dynamic.fileLabel}s</label>
              <div className="relative">
                ...
                >
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    <svg className="w-8 h-8 mb-3 text-secondary/40 group-hover:text-primary transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                    <p className="mb-2 text-sm font-bold text-secondary">{screenshots.length > 0 ? `${screenshots.length} files selected` : "Upload Screenshots"}</p>
                    <p className="text-xs text-secondary/50">PNG, JPG up to 10MB</p>
                  </div>
                </label>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <label className="text-sm font-black uppercase tracking-wider text-secondary/60">Phishing Attachments</label>
              <div className="relative">
                <input 
                  type="file" 
                ...
                >
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="mb-2 text-sm font-bold text-secondary">{attachments.length > 0 ? `${attachments.length} files selected` : "Upload Files"}</p>
                    <p className="text-xs text-secondary/50">.apk, .pdf, .zip etc.</p>
                  </div>
                </label>
              </div>
            </div>
