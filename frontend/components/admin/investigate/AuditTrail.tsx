...
function getActionColor(log: TicketAuditLog): {
  ...
  if (s.includes("mitigated"))
    return { bg: "bg-cyan-50", border: "border-cyan-200", text: "text-cyan-600" };
  if (s.includes("closed"))
  ...
  return { bg: "bg-neutral-page", border: "border-neutral-border", text: "text-secondary" };
}
        ...
        <div className="relative">
          ...
          <div className="space-y-0">
            {logs.map((log, index) => {
              ...
              return (
                <div key={log.id} className={`flex gap-3 ${!isLast ? "pb-4" : ""}`}>
                  {/* Icon node */}
                  <div
                  ...
                  <div className="flex-1 min-w-0 pt-0.5">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {log.new_status && (
                        <span
                          className={`flex-shrink-0 text-[10px] font-bold tracking-wide px-2 py-0.5 rounded-full border ${color.bg} ${color.border} ${color.text}`}
                        >
                          {log.new_status}
                        </span>
                      )}
