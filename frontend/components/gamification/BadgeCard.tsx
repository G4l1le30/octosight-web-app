...
export default function BadgeCard({ name, description, icon_url, earned, points }: BadgeCardProps) {
  return (
    <div className={`relative p-4 rounded-xl border text-center transition-all ${earned ? "bg-white border-primary/30 shadow-sm" : "bg-neutral-page border-neutral-border opacity-60"}`}>
      <div className="text-3xl mb-2">{icon_url || (earned ? "🏆" : "🔒")}</div>
      <h4 className="text-sm font-bold text-secondary">{name}</h4>
      <p className="text-xs text-secondary/60 mt-1">{description}</p>
      <span className="inline-block mt-2 text-xs font-bold px-2 py-0.5 rounded bg-primary/10 text-primary">{points} pts</span>
