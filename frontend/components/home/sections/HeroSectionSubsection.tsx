...
export const HeroSectionSubsection = () => {
  return (
    <section className="relative w-full bg-white px-6 py-12 md:px-8 md:pt-16 md:pb-24">
      <div className="mx-auto grid h-fit w-full max-w-screen-xl grid-cols-1 gap-8 md:gap-12 lg:grid-cols-2 lg:items-center">
        ...
        <article className="relative flex w-full flex-col items-start order-2 lg:order-1">
          <header className="relative flex w-full flex-col items-start">
            <h1 className="self-stretch text-4xl leading-[44px] md:text-5xl md:leading-[56px] lg:text-6xl lg:leading-[60px] font-normal tracking-[0]">
              <span className="font-black text-gray-900">
                Proactive Phishing Detection
                <br />
              </span>
              <span className="font-black bg-gradient-to-r from-[#e11d2e] via-[#c41525] to-[#ff5566] bg-clip-text text-transparent">
                Powered by Hybrid Risk Scoring
              </span>
            </h1>
          </header>

          <div className="relative mt-4 md:mt-6 flex w-full max-w-lg flex-col items-start">
            <p className="text-base md:text-lg leading-[26px] md:leading-[29.2px] font-normal tracking-[0] text-gray-500">
                ...
                <Button
                  ...
                  className={
                    action.variant === "primary"
                      ? "relative h-auto rounded-full bg-[#e11d2e] px-6 py-3 md:px-8 md:py-[17px] text-white hover:bg-[#e11d2e]/90 gap-2"
                      : "h-auto rounded-full border border-solid border-gray-200 bg-white px-6 py-[13px] md:px-8 md:py-4 text-gray-700 hover:bg-gray-50"
                  }
                >
                  {action.variant === "primary" && (
                    <span className="absolute inset-0 rounded-full bg-[#ffffff01] shadow-[0px_8px_10px_-6px_#fecaca,0px_20px_25px_-5px_#fecaca]" />
                  )}
                  <span className="relative flex w-fit items-center justify-center whitespace-nowrap text-center text-sm md:text-base leading-6 font-bold tracking-[0]">
                    {action.label}
                  </span>
                  {action.variant === "primary" && (
