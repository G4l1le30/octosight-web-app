        ...
        <div className="p-8">
            ...
            <div className="text-right">
              ...
              <div className="mt-1">
                <span className={`px-3 py-1 rounded-lg text-sm font-black border ${
                  result.status.toLowerCase() === 'submitted' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                  result.status.toLowerCase() === 'in review' ? 'bg-orange-50 text-orange-700 border-orange-200' :
                  result.status.toLowerCase() === 'confirmed' ? 'bg-red-50 text-red-700 border-red-200' :
                  result.status.toLowerCase() === 'false positive' ? 'bg-green-50 text-green-700 border-green-200' :
                  result.status.toLowerCase() === 'mitigated' ? 'bg-cyan-50 text-cyan-700 border-cyan-200' :
                  result.status.toLowerCase() === 'closed' ? 'bg-gray-100 text-gray-700 border-gray-200' :
            ...
            <div className="space-y-6">
              ...
              {result.url && (
                <div>
                  ...
                  <p className="text-base font-medium break-all opacity-90">{result.url}</p>
                </div>
              )}
              {result.sender_numbers && (
                <div>
                  <p className="text-sm font-bold text-secondary">Reported Sender</p>
                  <p className="text-base font-medium truncate">{result.sender_numbers}</p>
                </div>
              )}
                <div>
                  <p className="text-sm font-bold text-secondary">User Summary</p>
                  <p className="text-sm font-medium text-secondary leading-relaxed mt-1">
                ...
                <div>
                  <p className="text-sm font-bold text-secondary mb-3">Evidence Screenshots</p>
              ...
              {result.attachment_names && (
                <div>
                  <p className="text-sm font-bold text-secondary mb-3">Attachments</p>
                  <div className="space-y-2">
                  ...
                  {(() => {
                    ...
                    try {
                      if (result.analysis_results) {
                        details = { ...details, ...JSON.parse(result.analysis_results) };
                      }
                    } catch (e) {
                    ...
                    return (
                      <ul className="text-sm space-y-3 font-bold opacity-90">
                        <li className="flex justify-between">
                          ...
                          <span className={details.typosquatting !== "Safe" && details.typosquatting !== "Verified Domain" ? "text-risk-high" : "text-green-600"}>
                        ...
                        <li className="flex justify-between">
                          ...
                          <span className={details.keywords !== "Clean" ? "text-risk-high" : "text-green-600"}>
                            {details.keywords}
                          </span>
                        </li>
                        <li className="flex justify-between">
                          <span>Malicious Attachment:</span> 
                          <span className={details.attachments !== "Clean" ? "text-risk-high" : "text-green-600"}>
                            {details.attachments}
                          </span>
                        </li>
                      </ul>
          ...
          <div className="pt-6 border-t border-neutral-border flex flex-col md:flex-row md:items-center justify-between gap-4">
            <p className="text-sm font-normal opacity-90">
              Submitted on {new Date(result.created_at).toLocaleDateString("en-US", {
                month: "long",
                day: "numeric",
                year: "numeric"
